...
function buildDeveloperNav(providerDomains: string[]): NavSection[] {
  ...
  const sections: NavSection[] = [
    ...
    { type: 'item', item: { to: ROUTES.consumer.dashboard, label: 'Dashboard', icon: LayoutDashboard, end: true } },

    { type: 'item', item: { to: ROUTES.consumer.catalog, label: 'API Catalog', icon: Search } },

    { type: 'item', item: { to: ROUTES.consumer.planner, label: 'Application Planner', icon: Sparkles } },

    { type: 'item', item: { to: ROUTES.consumer.subscriptions, label: 'My Subscriptions', icon: FileKey } },

    { type: 'item', item: { to: ROUTES.consumer.applications, label: 'My Applications', icon: AppWindow } },
  ...
  if (providerDomains.length > 0) {
    ...
    sections.push({ type: 'item', item: { to: ROUTES.provider.dashboard, label: 'Provider Dashboard', icon: LayoutDashboard, end: true } });

    sections.push({ type: 'item', item: { to: ROUTES.provider.myApis, label: 'My APIs', icon: Server } });

    sections.push({ type: 'item', item: { to: ROUTES.provider.register, label: 'Register API', icon: PlusCircle } });

    sections.push({ type: 'item', item: { to: ROUTES.provider.requests, label: 'Consumer Requests', icon: Users } });

  } else {

    sections.push({ type: 'item', item: { to: ROUTES.developer.requestProvider, label: 'Request Publisher Access', icon: KeyRound } });
        ...
        {Array.isArray(nav) && 'type' in (nav[0] ?? {})
          ...
          ? (nav as NavSection[]).map((section, idx) =>
              ...
              section.type === 'divider' ? (
                ...
                <p key={idx} className="px-3 pt-4 pb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    ...
                    cn(
                      ...
                      isActive ? 'bg-brand-green-light text-brand-green' : 'text-slate-600 hover:bg-slate-50',
