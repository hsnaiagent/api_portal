...
export function ApplicationsPage() {
  ...
  const remove = (app: Application) => {
    ...
    dispatch({ type: 'UPDATE_APPLICATION', payload: { application_id: app.application_id, patch: { status: 'deleted' } } });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button type="button" onClick={openCreate} className="rounded-lg bg-brand-green px-4 py-2 text-brand-white text-sm font-medium hover:bg-brand-green-dark">Register Application</button>
      </div>
      ...
      {apps.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-12 text-center space-y-3">
          ...
          <button type="button" onClick={openCreate} className="inline-block rounded-lg bg-brand-green px-4 py-2 text-brand-white text-sm font-medium hover:bg-brand-green-dark">
            ...
            <div key={app.application_id} className="rounded-xl border border-slate-200 bg-brand-white p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex gap-1">
                  <button type="button" onClick={() => openEdit(app)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-50 hover:text-brand-blue" aria-label={`Edit ${app.name}`}>
                    <Pencil className="h-4 w-4" />
                  </button>
                  <button type="button" onClick={() => remove(app)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-50 hover:text-red-600" aria-label={`Delete ${app.name}`}>
              ...
              <span className="inline-block mt-2 text-xs rounded-full bg-slate-100 px-2 py-0.5 capitalize">{app.environment}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-xl bg-brand-white p-6 space-y-4">
            ...
            <div>
              <label htmlFor="app-name" className="block text-sm font-medium mb-1">Name <span className="text-red-500">*</span></label>
              <input id="app-name" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Application name" className="w-full rounded-lg border px-3 py-2 text-sm" />
            </div>
            <div>
              <label htmlFor="app-desc" className="block text-sm font-medium mb-1">Short description</label>
              <input id="app-desc" value={draft.desc} onChange={(e) => setDraft({ ...draft, desc: e.target.value })} placeholder="Short description" className="w-full rounded-lg border px-3 py-2 text-sm" />
            </div>
            <div>
              <label htmlFor="app-ai-desc" className="block text-sm font-medium mb-1">Application description (for AI)</label>
              <textarea id="app-ai-desc" value={draft.appDesc} onChange={(e) => setDraft({ ...draft, appDesc: e.target.value })} placeholder="What it does, what data it needs..." rows={4} className="w-full rounded-lg border px-3 py-2 text-sm" />
            </div>
            <div>
              <label htmlFor="app-env" className="block text-sm font-medium mb-1">Environment</label>
              <select id="app-env" value={draft.environment} onChange={(e) => setDraft({ ...draft, environment: e.target.value as Environment })} className="w-full rounded-lg border px-3 py-2 text-sm">
                <option value="sandbox">Sandbox</option>
                <option value="production">Production</option>
              </select>
            </div>
            <div className="flex gap-2 justify-end">
              ...
              <button type="button" onClick={save} disabled={!draft.name.trim()} className="rounded-lg bg-brand-green px-4 py-2 text-brand-white text-sm disabled:opacity-50">{editingId ? 'Save' : 'Create'}</button>
            </div>
          </div>
        </div>
      )}
